#!/usr/bin/env python3
"""Cross-platform installation and video generation for the shipcast Skill."""
...
DEFAULT_ROOT = Path.home() / "shipcast"
...
RECOMMENDED_LLM_PROVIDERS = {
    ...
    "volcengine": (
        ...
        "https://www.volcengine.com/activity/ai618?utm_source=shipcast",
    ),
...
def log(message: str) -> None:
    ...
    print(f"[shipcast] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install shipcast and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"shipcast installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional shipcast CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "shipcast.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "shipcast-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid shipcast project was found")
        if root.exists():
            root.rmdir()
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "shipcast-Agent-Skill",
        },
    )
    try:
...
def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / "shipcast-video" / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / "shipcast-video"

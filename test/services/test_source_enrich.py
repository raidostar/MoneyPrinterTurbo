...
class TestGetJson(unittest.TestCase):
    ...
    def test_a_json_body_comes_back_parsed(self):
        with _Fetch([_response('{"a": 1}', content_type="application/json")]):
            self.assertEqual(enrich.get_json("https://x.test/a"), {"a": 1})

    def test_a_body_that_is_not_json_does_not_raise(self):
        """
        여기서 예외가 나면 저장소 하나를 못 읽은 데서 끝나지 않고, 오늘 후보를
        고르는 일 전체가 죽는다.
        """
        with _Fetch([_response("<html>502 Bad Gateway</html>")]):
            self.assertIsNone(enrich.get_json("https://x.test/a"))

    def test_it_goes_through_the_same_address_checks(self):
        with _Fetch([_response('{"a": 1}')], addresses={"x.test": "10.0.0.5"}) as fetch:
            self.assertIsNone(enrich.get_json("https://x.test/a"))
            fetch.get.assert_not_called()

    def test_a_caller_can_ask_for_a_smaller_body(self):
        """저장소 목록은 본문보다 크게 잡아야 하고, 본문은 그대로 두어야 한다."""
        with _Fetch([_response('{"a": 1}')]) as fetch:
            enrich.get_json("https://x.test/a", limit=1234)
        fetch.responses[0].raw.read.assert_called_once_with(1234, decode_content=True)
